class BugDumpSpec:
    """
    Specification for bug-related output variables in simulation dumps.
    ...
    Rationale for defaults:
        The default output is ``"all"`` rather than ``"location"`` because it is easier to notice an excessively
    ...
    def __init__(self, outspec_h5: str ="all", custom_h5: list[str] | None = None):
        if outspec_h5 not in BugDumpSpec._VALID_H5:
            raise ValueError(f'Unknown output specification "{outspec_h5}". Valid values are: {", ".join(BugDumpSpec._VALID_H5)}')
